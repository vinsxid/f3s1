...
async def list_admin_handler(helper: Helper, id_bot: int):
    ...
    pesan = "<b>👑 Owner</b>\n"
    pesan += "• ID: " + str(config.id_admin) + " | <a href='tg://user?id=" + str(config.id_admin) + "'>Owner</a>\n\n"
    if len(db.admin) > 0:
        pesan += "<b>👷 Daftar Admin</b>\n"
        ind = 1
        for i in db.admin:
...
async def help_handler(client, msg):
    ...
    if member.status == 'owner':
        pesan += '\n⛑️ OWNER COMMAND\n'
        pesan += '/tf_coin — transfer coin\n'
        pesan += '/setting — melihat settingan bot\n'
        pesan += '/list_admin — melihat list admin\n'
        pesan += '/list_ban — melihat list banned\n'
